...
**Grenzen:** `ω_min` und `ω_max` werden nicht aus der Aufgabe erfunden. Standardglieder und asymptotische Darstellungen hängen von erkannter Zerlegung ab. Bei einer nicht unterstützten Zerlegung nennt LaTeX das betroffene Glied beziehungsweise den Fall, erklärt den Abbruch der vollständigen Standardgliederanalyse und weist auf weiter verwendbare numerische Frequenzgangresultate hin. Die Ansicht erklärt ausdrücklich, dass die numerischen Worked Steps keine vollständige asymptotische Bode-Konstruktion sind.
